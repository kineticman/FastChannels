...
# ("sdCallSignPairs" in the ah4c grabber's directv_grabber_stationids.json).
_SD_CALLSIGN_PAIRS = {'AHCH': 'AHCHD'}
...
def _collapse_hd_pairs(group: list[dict]) -> tuple[list[dict], dict[str, str], dict[str, str]]:
    ...
    def counterparts(hd: dict) -> list[dict]:
        ...
        if by_name:
            return by_name
        by_call_sign = [s for s in sd_rows if _call_sign_base(s) == _call_sign_base(hd)]
        if by_call_sign:
            return by_call_sign
        return [s for s in sd_rows if _SD_CALLSIGN_PAIRS.get(_call_sign(s)) == _call_sign(hd)]

...
def generate_streamvault_m3u(filters: dict = None, base_url: str = None,
                              feed_chnum_start: int = None, namespace_start: int = None,
                              feed_id: int = None) -> str:
    """
    StreamVault plugin feed (app/streamvault_plugin/): same DRM-bridge channel union as
    generate_kodi_bridge_m3u (base 'active' selection excludes anything requires_drm_bridge,
    so trusted-source DRM channels are unioned back in explicitly — see that function's
    docstring), but with #KODIPROP lines (like generate_m3u) and the generic
    /play/<source>/<id>.m3u8 play URL instead of the Kodi-bridge trigger URL — the
    StreamVault plugin resolves DRM channels itself via its own dash.mpd/license proxy
    lookups, it doesn't go through the Kodi/HDMI-encoder bridge at all.
    """
    filters  = filters or {}
    base_url = (base_url or '').rstrip('/')
    ...
    if feed_chnum_start is None and namespace_start is None:
        for w in warnings:
            log.warning('chnum overlap (streamvault): %s', w)
    else:
        _sort_by_assigned_chnum(channels, chnum_map)

...
class Source(db.Model):
    ...
    last_epg_success_at = db.Column(db.DateTime(timezone=True))  # last successful EPG program commit; distinct from last_scraped_at (stamped before EPG runs) so EPG-only failures stay visible
    ...
    def to_dict(self):
        return {
            ...
            'last_epg_success_at': self.last_epg_success_at.isoformat() if self.last_epg_success_at else None,
...
class TVEAccount(db.Model):
    ...
    def to_safe_dict(self):
        ...
        return {
            ...
            'auth_backend': auth_backend if auth_backend in {'native', 'yt_dlp'} else 'native',
            'adobe_mso_id': (cfg.get('adobe_mso_id') or 'Cox').strip(),
...
class Program(db.Model):
    ...
    series_id         = db.Column(db.String(64), nullable=True)
    episode_id        = db.Column(db.String(64), nullable=True)

...
| Tubi TV | Email/password required | **Default off.** Tubi no longer exposes its channel list to signed-out requests; a free Tubi account is required for channel discovery |
...
| Amazon Prime Free | Optional cookie header | **Default off.** Streams are DRM-only, bridged like the other DRM sources below |
...
- **DirecTV Stream**: resolves Gracenote IDs from its own API first (the `externalListingId` DirecTV attaches to every channel), with community-map fallback, and filters out non-streamable satellite-only lineup entries.
- **DirecTV Stream — Use DirecTV channel numbers**: off by default. On, every DirecTV channel is numbered in M3U output exactly as DirecTV numbers it (ESPN 206, CNN 202, ...) instead of getting an app-assigned number, so a DVR guide matches the DirecTV guide. Feeds that share one DirecTV number get subchannel suffixes: the main feed keeps the bare number, "... Alternate" feeds get sequential `.1`, `.2`, and feeds named with their own index ("WNBA on ION 2", "Big Ten Network Overflow 3") get that index (`305.2`, `610.3`). On satellite lineups the SD copy of an HD channel is left out so the pair does not claim the same number. Numbers are computed from your account's own lineup on every channel refresh. **Changing the toggle queues a channel refresh, and a stream audit runs automatically once that refresh has finished; wait for both before tuning, or the bridged channels will not play.** DirecTV's numbers can overlap another source's block; the app does not renumber around them.

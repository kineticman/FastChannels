...
- **DirecTV Stream / Sling**: both source cards have an **Exclude FAST channels** toggle to leave out free, ad-supported channels. Saving a change to this toggle on an enabled source queues a full scrape. That scrape marks excluded channels inactive without waiting for the normal miss-threshold grace period, while retaining their saved settings. DirecTV also resolves Gracenote IDs from its own API first, with community-map fallback, and filters out non-streamable satellite-only lineup entries.
